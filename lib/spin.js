...
exports.for = function (API) {
	...
	// TODO: Make this configurable via API.config
	const DEFAULT_LAST_COMMAND = "root.space.login";
//	const DEFAULT_LAST_COMMAND = "root.space.invite";
	...
	Instance.prototype.writeHeader = function () {
		var self = this;
		console.log(("Enter '<ProgramNumber> + [Return]' to restart program or 'commands' (list commands) or 'q' (quit):").cyan);
	...
	Instance.prototype.runREPL = function (callback) {
		...
		var lastCommand = DEFAULT_LAST_COMMAND;
...
//			}
			...
			if (data === "commands") {
				Object.keys(self._registeredCommands).forEach(function (commandAlias) {
					console.log("  " + commandAlias + " - " + self._registeredCommands[commandAlias].getLabel());
				});
				return;
			} else
			if (self._registeredCommands[data]) {
				console.log("Running command:", self._registeredCommands[data].getLabel());
				return API.Q.fcall(function () {
					return self._registeredCommands[data].run();
				}).fail(function (err) {
					if (err) {
						console.error("Error running command:", err.stack);
					}
				});
				return;
			}
